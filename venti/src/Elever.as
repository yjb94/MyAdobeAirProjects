package
{
	...
	{		
		...
		public static function loadEnviromentSavings(url:String):Object
		{
			var appStorage:File=File.applicationStorageDirectory;
			var result:Object = new Object;
			var final_result:Object  = new Object;
			for(var i : int = 0; i < 10; i++)
			{
				var dbFile:File = appStorage.resolvePath(url+i);
				if(dbFile.exists)
				{
					var fs:FileStream=new FileStream;
					fs.open(dbFile,FileMode.READ);
					result = JSON.parse(fs.readUTF());
					fs.close();
					for(var j:int = i*40; j < (i+1)*40; j++)
					{
		...
		public static function saveEnviromentSplit(url:String, obj:Object, split:int = 40):void
		{
			var appStorage:File = File.applicationStorageDirectory;
			var dbFile:File = appStorage.resolvePath(url);
			var fs:FileStream = new FileStream;
			...
			var strObj:Object = new Object;
			var index:int = 0;
			while(obj.length)
			{
				var object_30:Object = new Object;
				for(var i:int = 0; i < split; i++)
					object_30[i+(index*split)] = obj.shift();
			
				strObj[index] = JSON.stringify(object_30);
				if(index != 9)
				{
					strObj[index] = (strObj[index] as String).substr(0,  (strObj[index] as String).length-1) + ",";
				}
				if(index != 0)
				{
					strObj[index] = (strObj[index] as String).substr(1,  (strObj[index] as String).length);	
				}
				
				index++;
			}
			fs.writeUTF(strObj[0]+strObj[1]+strObj[2]+strObj[3]+strObj[4]+strObj[5]+strObj[6]+strObj[7]+strObj[8]+strObj[9]);
			fs.close();
		}
...
//			sprite.graphics.endFill();
		...
		private function onKeyDown(e:KeyboardEvent):void
		{
			...
			{
				if(_topLayer.getChildByName("popup") != null)
				{
