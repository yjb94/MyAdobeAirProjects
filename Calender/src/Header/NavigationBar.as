package Header
{
	...
	{		
		...
		public function set prevClicked(value:Boolean):void{ _isPrevClicked = value; }
...
//			Middle = null;
		...
		{
			if(Calender.Main.isChangingPage)	return;
			_isPrevClicked = true;
			Calender.Main.changePage(_prev_page[_prev_page.length-1].page_name, PageEffect.RIGHT, _prev_page[_prev_page.length-1].page_params);
			_prev_page.pop();
		}
		...
		public function set disable(value:Boolean):void
		{
			if(value)
			{
				Calender.Main.dictionary["NavigationBar"] = this;
				_saved_y = y;
				TweenLite.to(this, DISABLE_TWEEN_DURATION, { y:-y-this.height, alpha:0, onComplete:Disabled });
				Calender.Main.header.onResize();
			}
			else
			{
				Calender.Main.header.addChild(Calender.Main.dictionary["NavigationBar"]);
				Calender.Main.deleteItemInDict("NavigationBar");
				TweenLite.to(this, DISABLE_TWEEN_DURATION, { y:_saved_y, alpha:_alpha, onComplete:Abled });
			}
		}
		private function Disabled():void
		{
			Calender.Main.header.removeChild(Calender.Main.header.getChildByName("NavigationBar"));
		}
