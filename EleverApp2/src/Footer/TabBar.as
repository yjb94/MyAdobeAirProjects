...
//			_bars_page_param[_tabbed_index].y = Scroller.Y;
		...
		protected static const DISABLE_TWEEN_DURATION:Number = 0.5;
		public function set disable(value:Boolean):void
		{
			if(value)
			{
				Elever.Main.dictionary["TabBar"] = this;
				TweenLite.to(this, DISABLE_TWEEN_DURATION, { y:Elever.Main.FullHeight+this.height, alpha:0, onComplete:Disabled });
			}
			else
			{
				Elever.Main.footer.addChild(Elever.Main.dictionary["TabBar"]);
				Elever.Main.deleteItemInDict("TabBar");
				TweenLite.to(this, DISABLE_TWEEN_DURATION, { y:Elever.Main.FullHeight-this.height, alpha:_alpha, onComplete:Abled });
			}
		}
		private function Disabled():void
		{
			Elever.Main.footer.removeChild(Elever.Main.footer.getChildByName("TabBar"));
		}
