package Page.Home.Tabs
{
	...
	public class NowItem extends BasePage
	{
		...
		public function NowItem(params:Object=null)
		{
			super();
			...
			if(Elever.NowItemList != null)
			{
				for(var i:int = 0; i < Elever.NowItemList.length; i++)
				{
					makeItem(i);
				}
			}
		...
		private function makeItem(index:int):Sprite
		{
			...
			//loadImage
			loadImage(Elever.NowItemList[index].gathering_sub_image, spr, bmp, index);
			...
			spr.name = Elever.NowItemList[index].gathering_name;
		...
		private function onItemClick(e:MouseEvent=null):void
		{
			...
			else if(e.type == MouseEvent.MOUSE_UP)
			{
				...
				param.model = Elever.NowItemList[_item_spr_vector.indexOf(e.currentTarget as Sprite)];
				param.now_item = true;
		...
		private function loadImage(url:String, base_spr:Sprite, mask:Bitmap, index:int):void
		{
			...
			if(file.exists)
			{
				...
				loader.contentLoaderInfo.addEventListener(Event.COMPLETE, function(e:Event):void
				{
					...
					var txt:TextField = Text.newText(Elever.NowItemList[index].gathering_ymd, 27, 0xffffff, EXPLAIN_TEXT_LEFT_MARGIN);
					txt.y = explain_bmp.y + explain_bmp.height/2 - txt.height/2;
					base_spr.addChild(txt);
					
					txt = Text.newText(Elever.NowItemList[index].gathering_localname2, 27, 0xffffff);
