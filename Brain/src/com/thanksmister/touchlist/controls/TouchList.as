...
package com.thanksmister.touchlist.controls 
{
	...
	public class TouchList extends Sprite
	{
		...
		private var onListMoveCallback:Function = null;
		...
		//Getter / Setter
		public function get ScrollArea():Number{ return scrollAreaHeight; }
		public function get ScrollHeight():Number{ return scrollListHeight-scrollAreaHeight; }
		public function get ScrollTop():Number{ return -list.y; }
		public function set ScrollTop(value:Number):void{
			inertiaY=0;
			if(value<0) value=0;
			else if(value>ScrollHeight && ScrollHeight>0) value=ScrollHeight;
			if(value!=-list.y){
				list.y=(-value);
				updateList();
			}
		...
		public function get itemCount():int{ return list.numChildren; }
		...
		private function init(e:Event):void
		{
			...
			addEventListener(Event.ENTER_FRAME,onListTimer);
		...
		private function creatList():void
		{
			...
			list.graphics.clear();
			list.graphics.beginFill(0x000000, 0);
		...
		//리스트 크기 계산
		public function calcListSize():void{
			var currentItemIndex:int=-1;
			var currentItemPercent:Number=0;
			var prevListHeight:Number=scrollListHeight;
			
			scrollListHeight=0;
			for(var i:int=0;i<list.numChildren;i++){
				if(list.getChildAt(i).y-ScrollTop<ScrollArea && list.getChildAt(i).y+list.getChildAt(i).height-ScrollTop>=0){
					//크기를 조정하면서 원래위치가 틀어진다.. 정확한 위치를 정밀하게 기억하기 위해 아이템 번호와 해당 아이템이 표시안된 만큼 백분율을 구한다.
					currentItemIndex=i;
					currentItemPercent=(list.getChildAt(i).y+list.getChildAt(i).height-ScrollTop-ScrollArea)/list.getChildAt(i).height;
					if(currentItemPercent>1) currentItemPercent=1;
					else if(currentItemPercent<0) currentItemPercent=0;
				}
				
				list.getChildAt(i).y=scrollListHeight;
				scrollListHeight+=list.getChildAt(i).height;
			}
			
			if(prevListHeight!=scrollListHeight){
				if(currentItemIndex>=0){
					//원래 위치로 복귀
					ScrollTop=(list.getChildAt(currentItemIndex).y+list.getChildAt(currentItemIndex).height*(1-currentItemPercent))-ScrollArea;
				}
				createScrollBar();
				updateList();
			}
		...
		public function addListItemAt(item:ITouchListItemRenderer,index:int):void
		{
			...
			listItem.y = list.getChildAt(index).y;
			...
			list.addChildAt(listItem,index);
			
			for(var i:int=index+1;i<list.numChildren;i++){
				list.getChildAt(i).y+=listItem.height;
			}
		 ...
		 * */
		public function removeListItemAt(index:Number):void
		{
		...
		public function removeListItems():void
		{
			...
			while(list.numChildren > 0) {
				...
				if(item is Sprite){
					var itemSprite:Sprite=item as Sprite;
					while(itemSprite.numChildren>0) itemSprite.removeChildAt(0);
				}
				item.removeEventListener(ListItemEvent.ITEM_SELECTED, handleItemSelected);
				item.removeEventListener(ListItemEvent.ITEM_PRESS, handleItemPress);
			}
		...
		public function setOnListMove(fn:Function):void{
			onListMoveCallback=fn;
		}
		...
		private function updateList():void{
			for(var i:int=0;i<list.numChildren;i++){
				var item:DisplayObject=list.getChildAt(i);
				if(ScrollArea>0 && (item.y+item.height<ScrollTop || item.y>ScrollTop+ScrollArea))
					item.visible=false;
				else
					item.visible=true;
			}
			if(onListMoveCallback!=null)
				onListMoveCallback();
		}
		...
		private function onListTimer(e:Event):void
		{
			...
			if(!isTouching) {
				...
				scrollBar.scaleY=1.0;
				scrollBar.y = listHeight * Math.min(1, (-list.y/scrollListHeight) );
				if(scrollBar.y<0){
					scrollBar.height+=scrollBar.y;
					scrollBar.y=0;
				}
			}
		}
		...
		protected function destroy(e:Event):void
		{
			...
			tapDelayTime = 0;
			tapEnabled = false;
			//if(listTimer) listTimer.stop();
			//listTimer = null;
			removeEventListener(Event.ENTER_FRAME,onListTimer);
