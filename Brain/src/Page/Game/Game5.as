package Page.Game
{
	...
	public class Game5 extends BasePage
	{
		...
		{			
			...
			while(true)
			{
				var random_animal:int = rand(0, ANIMALS.length);
				if(random_animal != _animal_order[_game_index])
					break;
			}
			
			_animal_sound.load(new URLRequest("sound/"+_animal_sound_name[random_animal]));
		...
		public function gameOver():void
		{
			...
			if(!Brain.isTestGame && Brain.UserInfo.child_seq != -1)
			{
				...
				Brain.Connection.post("cognitiveGameAction5.cog", params, onLoadComplete);
			}
			else
