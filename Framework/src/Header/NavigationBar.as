...
//			Middle = null;
		...
		{
			if(Framework.Main.isChangingPage)	return;
			_isPrevClicked = true;
			Framework.Main.changePage(_prev_page[_prev_page.length-1].page_name, PageEffect.RIGHT, _prev_page[_prev_page.length-1].page_params);
			_prev_page.pop();
		}
		...
		public function set disable(value:Boolean):void
		{
			if(value)
			{
				Framework.Main.dictionary["NavigationBar"] = this;
				_saved_y = y;
				TweenLite.to(this, DISABLE_TWEEN_DURATION, { y:-y-this.height, alpha:0, onComplete:Disabled });
				Framework.Main.header.onResize();
			}
			else
			{
				Framework.Main.header.addChild(Framework.Main.dictionary["NavigationBar"]);
				Framework.Main.deleteItemInDict("NavigationBar");
				TweenLite.to(this, DISABLE_TWEEN_DURATION, { y:_saved_y, alpha:_alpha, onComplete:Abled });
			}
		}
		private function Disabled():void
		{
			Framework.Main.header.removeChild(Framework.Main.header.getChildByName("NavigationBar"));
		}
