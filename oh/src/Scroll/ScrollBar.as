package Scroll
{
	...
	{
		private static const TEMP:Number = 0.59259259
		private static const HEIGHT_MARGIN:Number = 10*TEMP;
		private static const WIDTH_MARGIN:Number = 8*TEMP;	//pixel
		private static const THICKNESS:Number = 8*TEMP;		//pixel
		private static const ROUNDNESS:Number = 10*TEMP;
		...
		{
			_width  = (width > Elever.Main.PageWidth) ? Elever.Main.PageWidth : width;		//set the scroll width
			_height = (height > Elever.Main.PageHeight) ? Elever.Main.PageHeight : height;	//set the scroll height
			onResize();
		}
		
		public var _scrollbar:Sprite;	//main scrollbar
		
		private var _total_height:Number;		//the total height that will scale the bar
		
		private var _prev_y:Number = Config.HEADER_SHADOW;
		private var _prev_height:Number = 0;
		private var _timeout_id:uint = 0;
		
		public function ScrollBar(width:Number, height:Number, scroller:Scroller)
		{
			super();
		...
		{			
			this.visible = Elever.Main.isChangingPage ? false : true;	//가독성을 위한 !연산자 안붙히고 논리연산자씀.
			...
			if(_scrollbar.y < Config.HEADER_SHADOW)
			{
				_scrollbar.scaleY = _height/(_total_height+(_scroller.y*SCROLL_BOUND_COEFFICIENT));
				_scrollbar.y = Config.HEADER_SHADOW;
			}
			else if(_scroller.Height-_scroller.height > _scroller.y)
			{
				_scrollbar.scaleY = _height/(_total_height+((_scroller.Height-_scroller.height-_scroller.y)*SCROLL_BOUND_COEFFICIENT));
		...
		{
			if(_scroller.height < _scroller.Height)
				return;
			
			_total_height = _scroller.height;		//sets the total height of scroll
