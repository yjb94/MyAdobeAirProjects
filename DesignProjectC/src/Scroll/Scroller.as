package Scroll
{
	...
	public class Scroller extends Sprite
	{
		...
		private const MOVE_PER_TOUCH:Number = 0.6;
		...
		private function onEnterFrame(e:Event):void
		{
			...
			if(this.height > _height)
			{
				...
				if(!(parent as Scroll).noScrollBar)
				...
				if(_doTween)	//Tween when dragged to top or bottom
				{
					...
					if(0 < y)
					{
						if(_start_y_zero)
							if(_func_up_slide)
								_func_up_slide();
						_scroll_tween = new TweenLite(this, AUTO_SCROLL_SPEED, {y:0, onComplete:tweenEnd });
						_isMoving = true;					//2014.11.27 isMoving함수 기능때문에 추가
						_doTween = false;
						_speed = 0;
						_savedSpeed = 0;
					}
					...
					{
						_scroll_tween = new TweenLite(this, AUTO_SCROLL_SPEED, {y:_height-height, onComplete:tweenEnd });
						_isMoving = true;					//2014.11.27 isMoving함수 기능때문에 추가
						_doTween = false;
						_speed = 0;
						_savedSpeed = 0;
		...
		{
			_isMoving = false;					//2014.11.27 isMoving함수 기능때문에 추가
		}
